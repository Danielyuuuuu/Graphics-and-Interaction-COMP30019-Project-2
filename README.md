...
All humanoid animation files (.fbx) are obtained from [Mixamo.com](https://mixamo.com). Then we create our own animator component to control the states for each animations and assign it to the script so that our characters can perform the appropriate animation in the game. 
